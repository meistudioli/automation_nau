...
dragonKeeper = {
	...
	catalysis: function(fileName, data) {
		...
		data = data.replace(/^\s*(.*)/gim, '        $1');
		data = data.replace(/^\s*(Scenario.*)/gim, '    $1');
		data = data.replace(/^\s*(@.*)/gim, '    $1');
		data = data.replace(/^\s*(Feature.*)/gim, '$1');
	...
	tenuto: function() {
		...
		seed.forEach(
			function(e, idx) {
				...
				comm += ' --require ' + dragonKeeper.stepDefinition;
				...
				.options(
					{
						...
						destination: {
							...
							help: 'set path for dragons, ex: ./cucumber/dragon'
						},
						tags: {
							abbr: 't',
