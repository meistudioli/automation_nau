...
singleEdit = function() {
    ...
    this.selector = {
        ...
        bargainSwitch: 'input[name="bargain"]',
        bargainRejectPrice: 'input[name="bargainAutoPrice[reject]"]',
...
singleEdit.prototype.turnBargainAs = function(mode) {
    var stand = this, m;
    m = (mode.toString() == '1') ? true : false;
    return this.one('bargainSwitch').getAttribute('checked').then(
        function(flag) {
            if ((m && !flag) || (flag && !m)) stand.one('bargainSwitch').click();
        }
    ).thenCatch(
        function() {
            //err catch
...
singleEdit.prototype.fillBargainRejectPrice = function(bargainRejectPrice) {
    var stand = this;
    return this.one('bargainSwitch').then(
        function() {
            stand.turnBargainAs('1');
        }
    ).then(
        function() {
            stand.one('bargainRejectPrice').clear().sendKeys(bargainRejectPrice);
        }
    ).thenCatch(
        function() {
            //err catch
...
singleEdit.prototype.goNext = function() {
    var stand = this, itemPreview = require(__base + constants.PO.itemPreview), uri;
